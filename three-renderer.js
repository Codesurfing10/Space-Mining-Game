// three-renderer.js — robust loader version
// This file dynamically imports three and its postprocessing modules. If imports fail (CSP/CDN blocked), it falls back to a safe no-op so the 2D game keeps running.

// Exposes window.renderThree(dt) and window.initThreeRenderer().

(function () {
  // default no-op so game can call safely
  window.renderThree = function () { /* 3D not available */ };
  window.initThreeRenderer = function () { /* 3D not available */ };

  async function bootstrap() {
    try {
      const THREEmod = await import('https://unpkg.com/three@0.158.0/build/three.module.js');
      ...
      function initThreeRendererIfNeeded() {
        ...
        if (typeof CFG === 'undefined') return; // game not initialized yet
        ...
        if (container) container.appendChild(threeState.renderer.domElement);
      ...
      function updateInstancedAsteroids() {
        if (!threeState.asteroidInst || !window.G) return;
        const count = Math.min(window.G.asteroids.length, threeState.maxAsteroids);
        let i = 0;
        for (; i < count; i++) {
          const a = window.G.asteroids[i];
      ...
      function updateParticles() {
        ...
        if (pp.geo.attributes.size) pp.geo.attributes.size.needsUpdate = true;
      }

      function renderThreeImpl(dt = 0) {
      ...
      // publish safe globals
      window.initThreeRenderer = initThreeRendererIfNeeded;
      window.renderThree = function (dt) {
        try { renderThreeImpl(dt); } catch (e) { console.error('3D render error, disabling 3D', e); window.renderThree = function(){}; }
      };

      console.log('Three renderer initialized (modules loaded)');
    } catch (err) {
      // If any import or init fails, keep 2D running — expose safe no-op functions.
      console.warn('three-renderer: failed to load 3D modules or initialize renderer — continuing with 2D only', err);
      window.renderThree = function () { };
      window.initThreeRenderer = function () { };
    }
  }

  // start bootstrap in background; does not block the page
  bootstrap();
})();
