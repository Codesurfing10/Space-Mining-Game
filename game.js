...
function render(dt) {
  ...
  if (laserBeam) {
    ...
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.moveTo(laserBeam.x1 - G.cam.x, laserBeam.y1 - G.cam.y);
    ctx.lineTo(laserBeam.x2 - G.cam.x, laserBeam.y2 - G.cam.y);
    ctx.stroke();
    ...
    ctx.beginPath();
    ctx.arc(laserBeam.hitX - G.cam.x, laserBeam.hitY - G.cam.y, 4, 0, Math.PI * 2);
    ctx.fill();
  }
